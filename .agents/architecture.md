...
Parquet shards are expanded to an image cache at startup by `HuggingFaceModelAssetService` using the C# `Parquet.Net` library — no external Python tooling is required. When the metadata row count in `metadata.jsonl` does not match the total row count across all Parquet shards, the cache is automatically rebuilt.

The runtime embedding cache is generated by the C# startup flow with the ONNX embedding model:
